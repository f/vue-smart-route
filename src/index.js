...
function flattenRoutes (routes = [], level = 0) {
  if (Array.isArray(routes)) {
    return routes.reduce((accumulator, { name, path, smart, tab, sidebar, children = [] }) => {
...
function smartSearch (value, context) {
  ...
  const matchingRoutes = smartRoutes.map(({ name, path, smart }) => {
    ...
    if (!smart.handler) { smart.handler = next }

    const matching = (
      typeof smart.matcher.search === 'function'
...
module.exports = {
  install: function () {
    Vue.default.directive('smart-results', {
      bind: function (el, binding, vnode) {
        var model = vnode.data.directives.filter(d => d.name === 'model')
        if (!model.length) {
          throw new Error('An input with v-smart-results directive must have v-model.')
        }
        vnode.context.$watch(model[0].expression, function (value) {
          this[binding.expression] = smartSearch(value, this)
        })
      }
    })
  }
